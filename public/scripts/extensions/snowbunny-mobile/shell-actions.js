...
function presetRecord(select = activePresetSelect()) {
    const api = context();
    if (!(select instanceof HTMLSelectElement) || !select.value) return null;
    return {
        mainApi: api?.mainApi || '',
        selectId: select.id,
        value: select.value,
...
function presetName(select = activePresetSelect()) {
...
function rememberCurrentPreset() {
    if (applyingPreset || !context()?.getCurrentChatId?.()) return;
    const record = presetRecord();
    if (record) snowState()?.patchChat?.({ presetSelection: record });
}

function applySavedPreset() {
    if (!context()?.getCurrentChatId?.()) return;
    const state = snowState()?.readChat?.();
    const saved = state?.presetSelection;
    if (!saved || typeof saved !== 'object') {
        rememberCurrentPreset();
        return;
    }

    const byId = typeof saved.selectId === 'string' ? document.getElementById(saved.selectId) : null;
    const select = byId instanceof HTMLSelectElement ? byId : activePresetSelect();
    if (!(select instanceof HTMLSelectElement)) return;
    const hasValue = [...select.options].some(option => option.value === saved.value && !option.disabled);
    if (!hasValue || select.value === saved.value) return;
...
function openPresetSelector() {
    ...
    const render = () => {
        ...
        for (const option of visible) {
            ...
            if (option.value === select.value) copy.append(el('small', '', 'Current preset for this chat'));
            button.append(copy);
            button.addEventListener('click', () => {
                ...
                snowState()?.patchChat?.({ presetSelection: presetRecord(select) });
                closeSheet();
...
function enhanceRows() {
    ...
    if (preset) {
        ...
        if (value) value.textContent = presetName();
...
function registerEvents() {
    ...
    if (types.PRESET_CHANGED) source.on(types.PRESET_CHANGED, presetChanged);
    if (types.OAI_PRESET_CHANGED_AFTER) source.on(types.OAI_PRESET_CHANGED_AFTER, presetChanged);
}
