...
const SHELL_STYLE_ID = 'snowbunny-shell-style';
...
@media screen and (max-width: 1000px) {
  ...
  /* Retire the stock icon parade, but leave the real ST drawers alive so
     SnowBunny can temporarily route to their backend controls while each
     dedicated mobile surface is ported. */
  ...
  .snowbunny-library-current strong, .snowbunny-library-current small {
  ...
  #${SHEET_ID} .snowbunny-shell-sheet-handle {
    width: 38px; height: 4px; margin: 1px auto 9px; border-radius: 99px;
...
function shellRow({ label, description = '', iconName, value = '', onClick, disabled = false }) {
    ...
    if (!disabled && onClick) {
        button.append(icon('fa-chevron-right')).classList.add?.('snowbunny-shell-row-chevron');
...
function openAppearanceSheet() {
    ...
    card.append(
        ...
        shellRow({
            ...
            description: 'Use SillyTavern’s current theme controls while SnowBunny’s appearance editor is ported.',
            iconName: 'fa-palette',
...
function installTopStrip() {
    ...
    items.append(
        topAction({ label: 'Stories', iconName: 'fa-book-open', disabled: true, title: 'Stories browser is being ported next' }),
        topAction({ label: 'Response', iconName: 'fa-sliders', onClick: () => openNativeDrawer('ai-config-button'), title: 'AI Response Configuration' }),
        topAction({ label: 'API', iconName: 'fa-plug', onClick: () => openNativeDrawer('sys-settings-button') }),
        topAction({ label: 'Codex', iconName: 'fa-book-atlas', disabled: true, title: 'SnowBunny Codex is not wired to this shell yet' }),
    ...
    toggle.addEventListener('click', () => {
        ...
        globalThis.SnowBunny?.state?.patchGlobal?.({ topMenuCollapsed: document.body.classList.contains('snowbunny-top-collapsed') });
    });
...
function currentChatLabel() {
    ...
    if (!chatId) return 'No chat open';
    const last = Array.isArray(api?.chat) ? api.chat.at(-1) : null;
    const partner = last?.is_user ? api?.name2 : last?.name || api?.name2;
    return partner || 'Current chat';
}

function currentMember() {
    const api = context();
    const assistant = [...document.querySelectorAll('#chat .mes:not([is_user="true"])')].reverse().find(message => {
    ...
    const name = data?.name || api?.name2 || 'Assistant';
    const src = assistant?.querySelector('.avatar img')?.getAttribute('src') || '';
    return { name, src };
}
...
function selectedPreset() {
    ...
    for (const selector of selectors) {
        ...
        if (!(select instanceof HTMLSelectElement) || select.offsetParent === null) continue;
...
function selectedModel() {
    ...
    try {
        ...
        // The active backend may be a text-completion API.
    }
    const candidates = [
        '#openai_model',
...
function registerRefreshEvents() {
    ...
    for (const name of [
        'CHAT_CHANGED', 'CHAT_LOADED', 'MESSAGE_RECEIVED', 'MESSAGE_SENT',
        'CHARACTER_EDITED', 'PERSONA_CHANGED', 'CHAT_COMPLETION_MODEL_CHANGED',
        'MAIN_API_CHANGED', 'PRESET_CHANGED', 'OAI_PRESET_CHANGED_AFTER',
    ]) {
