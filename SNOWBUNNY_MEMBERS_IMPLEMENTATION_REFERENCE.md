...
This file records how SnowBunny's already-settled Current Chat **Members** UX maps onto the SillyTavern fork. It is an implementation contract, not a redesign of the Members experience.
...
- SnowBunny Story ownership remains a separate layer. A Story is a collection/continuity owner for Lorebooks + Memories; it is not an ST group.
...
- `group.disabled_members` is used as SnowBunny's current participation/mute state for automatic replies.
...
These are backend implementation choices. They must remain subordinate to SnowBunny's prompt/context routing as the custom systems are ported.
...
This avoids corrupting the user's original ST chat while giving SnowBunny a real multi-member backend immediately.

The copied chat is a new chat identity. It is not a silent in-place mutation of the old file.

## Member picker

The right-drawer `Add` action opens SnowBunny's own mobile Character picker:
...
- multi-select;
- Apply commits the resulting ordinary Character member list.

For an existing group-backed chat, Apply edits the existing real group member array.

For a legacy single-Character chat, changing the selection triggers the safe promotion/copy flow above.

## Narrator is still special

Narrator must **not** be turned into a fake ST Character merely to fit `group.members`.

The settled Narrator rules remain:

- protected built-in SnowBunny identity;
- dedicated name, portrait, Narrator instructions, and Voice/style;
- selectable through the same Members picker for usability;
- may coexist with ordinary Characters;
- AI serialization identifies it as Narrator, not as a normal Character card;
- protected from deletion.

Therefore the final Members model is conceptually:

- ordinary Character members backed by ST group membership;
- optional SnowBunny Narrator member backed by the special Narrator resource.

The Narrator adapter is still to be implemented.

## Single-member new SnowBunny chats

For consistency, a future native SnowBunny new-chat flow may choose to create a group-backed chat even when it starts with one ordinary Character, so adding a second Character does not require promotion later.
...
Narrator-only and Narrator-plus-Character starts require the special Narrator adapter and must not be forced through a fake Character card.
...
- infer member identity by name when stable Character identity is available;
- call an ST group a SnowBunny Story;
- apply Story-wide ownership to Members;
- flatten Narrator into a normal Character card;
- claim the mute/participation icon is a verified copy of Tavo's unknown control semantics;
